...
export function FeatureShowcase({ onNavigateToTool }: { onNavigateToTool?: (mode: ToolMode) => void }) {
  const handleClick = (mode: ToolMode) => {
    onNavigateToTool?.(mode);
    document.getElementById("tools")?.scrollIntoView({ behavior: "smooth" });
  };

  const features: { id: ToolMode; icon: React.ReactNode; title: string; desc: string }[] = [
    {
      id: "single",
      icon: (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 stagger">
          {features.map((f, i) => (
            <button
              ...
              className="p-6 rounded-[var(--radius-lg)] bg-[var(--bg-card)] border border-[var(--border)] hover:border-[var(--border-hover)] transition-colors shine text-left w-full"
            >
