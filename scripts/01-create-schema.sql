...
-- Junction table linking analyses to repositories (many-to-many)
...
CREATE TABLE IF NOT EXISTS app_blueprints (
  ...
  reuse_percentage INTEGER DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_analysis_repositories_repository_id ON analysis_repositories(repository_id);
CREATE INDEX IF NOT EXISTS idx_app_blueprints_analysis_id ON app_blueprints(analysis_id);
CREATE INDEX IF NOT EXISTS idx_app_blueprints_reuse ON app_blueprints(reuse_percentage DESC);
